...
def test_transaction_history_structure():
    ...
    assert isinstance(
        transaction_history["transaction_type_sold"], bool
    ), "transaction_type_sold must be a boolean"
...
def test_mark_as_sold_logic():
    ...
    assert (
        transaction_history["transaction_type_sold"] == True
    ), "transaction_type_sold should be True when marked as sold"
...
def test_mark_as_active_logic():
    ...
    assert (
        transaction_history["transaction_type_sold"] == False
    ), "transaction_type_sold should be False when marked as active"
...
def test_transaction_history_toggle():
    ...
    # Verify the sequence
    assert (
        transaction_history_records[0]["transaction_type_sold"] == True
    ), "First record should be sold"
    assert (
        transaction_history_records[1]["transaction_type_sold"] == False
    ), "Second record should be active"
    assert (
        transaction_history_records[2]["transaction_type_sold"] == True
    ), "Third record should be sold"
