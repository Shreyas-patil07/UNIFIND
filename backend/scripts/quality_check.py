...
def main():
    ...
    # 1. Ruff linter
    results["Ruff Linter"] = run_command("Ruff Linter", ["ruff", "check", "."], check=False)

    # 2. Black formatter check
    results["Black Formatter"] = run_command(
        "Black Formatter Check", ["black", "--check", "--diff", "."], check=False
    )

    # 3. isort import check
    results["isort Import Check"] = run_command(
        "isort Import Check", ["isort", "--check-only", "--diff", "."], check=False
    )
    ...
    results["Bandit Security Scan"] = run_command(
        "Bandit Security Scan", ["bandit", "-r", "app/", "-ll", "-f", "screen"], check=False
    )

    # 6. Safety dependency check
    results["Safety Dependency Check"] = run_command(
        "Safety Dependency Check", ["safety", "check", "--file", "requirements.txt"], check=False
    )
