...
class UserRepository(UserRepositoryInterface):
    ...
    async def delete(self, user_id: str) -> bool:
        ...
        if profile:
            ...
            if profile_id:
                self.profiles_collection.document(profile_id).delete()
        
        return True

    async def get_batch(self, user_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        Batch fetch users by IDs (optimized for N+1 query prevention).
        ...
        for i in range(0, len(user_ids), 10):
            ...
            for user_id in batch_ids:
                ...
                if doc.exists:
                    user_data = doc.to_dict()
                    user_data["id"] = doc.id
                    users_map[doc.id] = user_data
        
        return users_map
    
    async def get_profiles_batch(self, user_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        Batch fetch profiles by user IDs (optimized for N+1 query prevention).
        ...
        for i in range(0, len(user_ids), 10):
            batch_ids = user_ids[i:i+10]
            query = self.profiles_collection.where("user_id", "in", batch_ids)
    ...
    async def get_profile_with_user(
        ...
        if not profile_data:
            ...
            profile_data = {
                ...
                "review_count": 0,
                "member_since": (
                    user_data.get("created_at", "").split("T")[0].split("-")[0]
        ...
        combined_data = {**profile_data, **user_data, "user_id": user_id}
