...
from google.cloud.firestore import DELETE_FIELD, ArrayUnion, Increment
...
class ProductRepository(ProductRepositoryInterface):
    ...
    async def get_all(
        ...
        """
        Get all products with optional filters and pagination (OPTIMIZED).
        
        PERFORMANCE: Pushes filters to Firestore instead of Python filtering.
        Requires composite indexes for multi-field queries.
        """
        query = self.collection

        # Apply Firestore filters (OPTIMIZATION: filter at DB level)
        if category:
        ...
        # Price range filters (requires composite index)
    ...
    async def increment_view(self, product_id: str, user_id: str) -> bool:
        ...
        if user_id not in viewed_by:
            doc_ref.update({"viewed_by": ArrayUnion([user_id]), "views": Increment(1)})
            return True
    ...
    async def mark_as_sold(self, product_id: str, buyer_id: Optional[str] = None) -> bool:
        ...
        update_data = {"is_active": False, "sold_at": datetime.now(), "updated_at": datetime.now()}
