...
class ReviewRepository:
    ...
    async def get_by_user(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        query = (
            self.collection.where("reviewer_id", "==", user_id)
    ...
    async def get_for_user(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        query = (
            self.collection.where("reviewed_user_id", "==", user_id)
            .order_by("created_at", direction="DESCENDING")
            .limit(limit)
        )
    ...
    async def get_by_product(self, product_id: str) -> List[Dict[str, Any]]:
        ...
        query = self.collection.where("product_id", "==", product_id).order_by(
    ...
    async def check_existing_review(
        self, reviewer_id: str, reviewed_user_id: str, product_id: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """Check if a review already exists."""
        query = self.collection.where("reviewer_id", "==", reviewer_id).where(
