...
from app.ai.clients.gemini_client import GeminiAPIError, GeminiTimeoutError
from app.ai.prompts.intent_extractor import extract_intent
from app.ai.ranking.semantic_ranker import rank_listings
from app.api.dependencies.auth import get_current_user
from app.core.database import get_db
from app.schemas.need import ExtractedIntent, NeedBoardRequest, NeedBoardResponse, RankedResult

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/need-board", tags=["need-board"])
...
def get_user_profile_and_searches(db, user_id: str, current_time: float):
    """
    ...
    cleaned_searches = [
        s for s in need_board_searches if s.get("timestamp", 0) > twenty_four_hours_ago
    ]

    # Update profile if we cleaned anything
    if len(cleaned_searches) != len(need_board_searches):
        profile_doc.reference.update({"need_board_searches": cleaned_searches})
...
async def get_search_history(current_user: str = Depends(get_current_user)):
    ...
    if profile_ref is None:
        return {"searches": [], "searches_remaining": MAX_SEARCHES_PER_PERIOD}
...
@router.post("", response_model=NeedBoardResponse)
async def need_board(
    request: NeedBoardRequest, req: Request, current_user: str = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        # Pass user_id for cost tracking
        intent = await extract_intent(request.query, user_id=user_id)
        logger.debug(f"Extracted intent: {intent}")

        # Fetch active products from database with pre-filtering (OPTIMIZED: reduced limit)
        ...
        if extracted_category and extracted_category != "Other":
            ...
            query = query.where("category", "==", extracted_category).limit(30)  # OPTIMIZED: 50 → 30
        else:
            query = query.limit(50)  # OPTIMIZED: 100 → 50
        ...
        # Pass user_id for cost tracking
        ranked = await rank_listings(request.query, intent, listings, user_id=user_id)
        ...
        logger.info(
            f"Successfully processed query with {len(enriched)} total results, {len(filtered_enriched)} results >= 50% (showing top 10)"
        )
        ...
        logger.info(
            f"Saving search record with {len(search_record['results'])} results (>= 50% match, top 10)"
        )
        ...
        profile_ref.update({"need_board_searches": need_board_searches})
