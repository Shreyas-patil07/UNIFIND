...
from app.schemas.need import NeedCreate, NeedFulfillRequest, SellerDemandBanner, SellerNeedFeed
...
async def fulfill_need(
    ...
    try:
        success = await need_service.fulfill_need(need_id, request.product_id, current_user)
        if success:
            return {"message": "Need marked as fulfilled"}
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update need"
        )
    except ValueError as e:
        if "Not authorized" in str(e):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
...
async def save_need(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save need"
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
