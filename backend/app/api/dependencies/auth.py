...
async def verify_firebase_token(request: Request) -> str:
    ...
    if not auth_header.startswith("Bearer "):
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid authentication token format"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication token has been revoked"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Failed to verify authentication token"
        )
