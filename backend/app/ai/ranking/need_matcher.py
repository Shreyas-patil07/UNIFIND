...
def match_need_to_listings(need: Dict, listings: List[Dict], limit: int = 10) -> List[Dict]:
    """
...
def match_need_to_sellers(need: Dict, sellers: List[Dict], limit: int = 20) -> List[str]:
    """
    ...
    for seller in sellers:
        ...
        if total_score > 0:
            ...
            seller_scores.append({"user_id": seller_id, "score": final_score})
...
def rank_needs_for_seller(
    seller_listings: List[Dict], needs: List[Dict], limit: int = 10
) -> List[Dict]:
    """
    ...
    for listing in seller_listings:
        seller_keywords.update(
            extract_keywords(f"{listing.get('title', '')} {listing.get('description', '')}")
        )
    ...
    for need in needs:
        ...
        if score >= 30:  # Minimum threshold
            ranked_needs.append({**need, "relevance_score": score})
