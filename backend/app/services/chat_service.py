...
class ChatService:
    ...
    def __init__(self, chat_repo: ChatRepository, friendship_repo: FriendshipRepository):
        self.chat_repo = chat_repo
        self.friendship_repo = friendship_repo

    def _generate_chat_room_id(
        self, user1_id: str, user2_id: str, product_id: Optional[str] = None
    ) -> str:
    ...
    async def send_message(
        self, message_data: MessageCreate, authenticated_user: str
    ) -> Dict[str, Any]:
        """
        ...
        chat_room_id = self._generate_chat_room_id(
            message_data.sender_id, message_data.receiver_id, message_data.product_id
        )
        ...
        if not chat_room:
            ...
            chat_room_data = {
                ...
                "unread_count_user1": (
                    ...
                    == min(message_data.sender_id, message_data.receiver_id)
                    else 0
                ),
                "unread_count_user2": (
                    1
                    if message_data.receiver_id
                    == max(message_data.sender_id, message_data.receiver_id)
                    else 0
                ),
            }
            ...
            unread_field = (
                "unread_count_user1"
                if message_data.receiver_id == chat_room["user1_id"]
    ...
    async def get_user_chats(
        self, user_id: str, friends_only: bool = False
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_chat_messages(
        self, chat_room_id: str, authenticated_user: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        is_participant = await self.chat_repo.verify_chat_participant(
            chat_room_id, authenticated_user
        )
    ...
    async def mark_messages_as_read(self, chat_room_id: str, user_id: str) -> int:
        """
        ...
        unread_field = (
            "unread_count_user1" if user_id == chat_room["user1_id"] else "unread_count_user2"
        )
